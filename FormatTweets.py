...
parser.add_argument('-i', '--input', action='store', dest='input', default=Database.tweets['table_name'], help='The name of the input table, which should have columns "id" and "text", where id is a big int representing the tweet id, and text is a string of some sort.')
parser.add_argument('-o', '--output', action='store', dest='output', default=Database.formatted_tweets['table_name'], help='The name of the output table, which will have columns "id", and "tokens", where id will be the same as the input id, and tokens will be a tokenized and formatted array of strings.')
...
parser.add_argument('-iic', '--input_id_column', action='store', dest='in_id_column', default=Database.tweets['id_column'], help='The name of the input column holding the tweet ids.')
parser.add_argument('-itc', '--input_text_column', action='store', dest='in_text_column', default=Database.tweets['text_column'], help='The name of the input column holding the tweet ids.')
parser.add_argument('-oic', '--output_id_column', action='store', dest='out_id_column', default=Database.formatted_tweets['id_column'], help='The name of the output column to which the tweet ids will be written.')
parser.add_argument('-otc', '--output_token_column', action='store', dest='out_token_column', default=Database.formatted_tweets['tokens_column'], help='The name of the output column to which the tokens will be written.')
...
for row in incur:
    ...
    if incur.rownumber % 1000 == 1:#int(incur.rowcount / 100) == 0:
        ...
        outcur.execute("""COMMIT""")
        print str(int(incur.rownumber / incur.rowcount)) + "% complete. Est. completion time: " + time.strftime("%b %d %Y %H:%M:%S", time.localtime(fin))
