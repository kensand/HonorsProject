...
for id, embedding in l:
    ...
    cur2.execute("""SELECT word FROM dictionary WHERE word_id = %s""", [id])
    out = str(cur2.fetchone()) + '->'
    cur.execute("""SELECT word_id, word_embedding FROM word_embeddings WHERE word_id != %s""", [id])
    ...
    for i,j in rel.most_common(10):
        cur2.execute("""SELECT word FROM dictionary WHERE word_id = %s""", [i])
